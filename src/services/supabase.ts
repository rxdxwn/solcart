...
const DEFAULT_STAFF = [
  { id: "staff-1", name: "Sarah Owner", email: "owner@solcart.shop", role: "Owner", permissions: ["*"], createdAt: "2025-01-10T12:00:00Z", lastActive: "Just now", status: "Active", assignedOrders: 1 },
  { id: "staff-2", name: "Alex SuperAdmin", email: "superadmin@solcart.shop", role: "Super Admin", permissions: ["*"], createdAt: "2025-01-11T12:00:00Z", lastActive: "Just now", status: "Active", assignedOrders: 0 },
  { id: "staff-3", name: "Fred Finance", email: "finance@solcart.shop", role: "Finance Manager", permissions: ["overview", "analytics", "payments", "refunds", "finance"], createdAt: "2025-01-12T12:00:00Z", lastActive: "2 hours ago", status: "Active", assignedOrders: 0 },
  { id: "staff-4", name: "Olivia Operations", email: "ops@solcart.shop", role: "Operations Manager", permissions: ["overview", "orders", "retailers", "products", "inventory", "notifications", "settings"], createdAt: "2025-01-13T12:00:00Z", lastActive: "1 day ago", status: "Active", assignedOrders: 2 },
  { id: "staff-5", name: "Steve Support", email: "support@solcart.shop", role: "Customer Support", permissions: ["orders", "refunds", "support"], createdAt: "2025-01-14T12:00:00Z", lastActive: "5 mins ago", status: "Active", assignedOrders: 1 },
  { id: "staff-6", name: "Frank Fulfillment", email: "fulfillment@solcart.shop", role: "Fulfillment Manager", permissions: ["orders", "inventory"], createdAt: "2025-01-15T12:00:00Z", lastActive: "3 days ago", status: "Active", assignedOrders: 1 },
  { id: "staff-7", name: "Ana Analyst", email: "analyst@solcart.shop", role: "Read-Only Analyst", permissions: ["overview", "analytics", "customers", "payments", "refunds", "retailers", "products", "inventory", "finance"], createdAt: "2025-01-16T12:00:00Z", lastActive: "4 hours ago", status: "Active", assignedOrders: 0 }
];
...
export class SupabaseService {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (result.success && result.data) {
            ...
            if (result.data.giftCardPool) {
              localStorage.setItem("solcart_db_gift_card_pool", JSON.stringify(result.data.giftCardPool));
            }
            if (result.data.launches) {
              localStorage.setItem("solcart_db_launches", JSON.stringify(result.data.launches));
            }
  ...
  static getGiftCardPool(): any[] {
    if (typeof window === "undefined") return [];
    const stored = localStorage.getItem("solcart_db_gift_card_pool");
    if (!stored) {
      const INITIAL_GC_INVENTORY: Record<string, Record<string, Record<number, string[]>>> = {
        "amazon": {
          "United States": {
            25: ["AMZN-US-25-A1B2C3D4", "AMZN-US-25-E5F6G7H8"],
            50: ["AMZN-US-50-I9J0K1L2", "AMZN-US-50-M3N4O5P6"],
            100: ["AMZN-US-100-Q7R8S9T0", "AMZN-US-100-U1V2W3X4"]
          },
          "United Kingdom": {
            25: ["AMZN-UK-25-G1H2I3J4", "AMZN-UK-25-K5L6M7N8"],
            50: ["AMZN-UK-50-O9P0Q1R2", "AMZN-UK-50-S3T4U5V6"]
          },
          "Singapore": {
            50: ["AMZN-SG-50-X1Y2Z3A4", "AMZN-SG-50-B5C6D7E8"]
          },
          "Canada": {
            50: ["AMZN-CA-50-F1G2H3I4", "AMZN-CA-50-J5K6L7M8"]
          }
        },
        "apple": {
          "United States": {
            50: ["APPL-US-50-Z9Y8X7W6", "APPL-US-50-V5U4T3S2"],
            100: ["APPL-US-100-R1Q2P3O4", "APPL-US-100-N5M6L7K8"]
          },
          "Singapore": {
            100: ["APPL-SG-100-J1I2H3G4", "APPL-SG-100-F5E6D7C8"]
          }
        },
        "noon": {
          "United States": {
            50: ["NOON-US-50-ABC111", "NOON-US-50-XYZ222"],
            100: ["NOON-US-100-NOON333", "NOON-US-100-NOON444"]
          },
          "Singapore": {
            100: ["NOON-SG-100-NOONSG55", "NOON-SG-100-NOONSG66"]
          }
        },
        "nike": {
          "United States": {
            50: ["NIKE-US-50-SWOOSH1", "NIKE-US-50-SWOOSH2"],
            100: ["NIKE-US-100-JUSTDOIT1", "NIKE-US-100-JUSTDOIT2"]
          }
        },
        "steam": {
          "United States": {
            20: ["STEAM-US-20-VALVE1", "STEAM-US-20-VALVE2"],
            50: ["STEAM-US-50-GABEN1", "STEAM-US-50-GABEN2"]
          }
        }
      };
      ...
      let idCounter = 1;
      for (const brand in INITIAL_GC_INVENTORY) {
        for (const region in INITIAL_GC_INVENTORY[brand]) {
          for (const priceStr in INITIAL_GC_INVENTORY[brand][region]) {
            const price = parseInt(priceStr);
            const codes = INITIAL_GC_INVENTORY[brand][region][price];
            codes.forEach(code => {
              initialPool.push({
                id: `gc-${idCounter++}`,
      ...
      localStorage.setItem("solcart_db_gift_card_pool", JSON.stringify(initialPool));
      return initialPool;
    }
    try {
      return JSON.parse(stored);
  ...
  static saveGiftCardPool(pool: any[]): void {
    if (typeof window === "undefined") return;
    localStorage.setItem("solcart_db_gift_card_pool", JSON.stringify(pool));
    
    // Sync with central database api if desired
    fetch("/api/db", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "saveGiftCardPool",
        payload: pool
      })
    }).catch(() => {});
  }

  static getLaunches(): any[] {
    if (typeof window === "undefined") return [];
    const stored = localStorage.getItem("solcart_db_launches");
    if (!stored) {
      const defaultLaunches = [
        { country: "United States", code: "US", currency: "USD", symbol: "$", rateToUsd: 1.0, launchDate: "Active Now", status: "active", weekOffset: 0 },
        { country: "United Kingdom", code: "GB", currency: "GBP", symbol: "£", rateToUsd: 1.28, launchDate: "Active Now", status: "active", weekOffset: 0 },
        { country: "Singapore", code: "SG", currency: "SGD", symbol: "S$", rateToUsd: 0.74, launchDate: "Active Now", status: "active", weekOffset: 0 },
        { country: "Canada", code: "CA", currency: "CAD", symbol: "C$", rateToUsd: 0.73, launchDate: "Active Now", status: "active", weekOffset: 0 },
        { country: "Germany & Eurozone", code: "DE", currency: "EUR", symbol: "€", rateToUsd: 1.09, launchDate: "August 20, 2026", status: "upcoming", weekOffset: 1 },
        { country: "Japan", code: "JP", currency: "JPY", symbol: "¥", rateToUsd: 0.0065, launchDate: "August 27, 2026", status: "upcoming", weekOffset: 2 },
        { country: "Australia", code: "AU", currency: "AUD", symbol: "A$", rateToUsd: 0.65, launchDate: "September 3, 2026", status: "upcoming", weekOffset: 3 },
        { country: "United Arab Emirates", code: "AE", currency: "AED", symbol: "د.إ", rateToUsd: 0.272, launchDate: "September 10, 2026", status: "upcoming", weekOffset: 4 },
        { country: "India", code: "IN", currency: "INR", symbol: "₹", rateToUsd: 0.012, launchDate: "September 17, 2026", status: "upcoming", weekOffset: 5 },
        { country: "Brazil", code: "BR", currency: "BRL", symbol: "R$", rateToUsd: 0.18, launchDate: "September 24, 2026", status: "upcoming", weekOffset: 6 },
        { country: "Switzerland", code: "CH", currency: "CHF", symbol: "CHF", rateToUsd: 1.13, launchDate: "October 1, 2026", status: "upcoming", weekOffset: 7 },
        { country: "South Korea", code: "KR", currency: "KRW", symbol: "₩", rateToUsd: 0.00073, launchDate: "October 8, 2026", status: "upcoming", weekOffset: 8 }
      ];
      localStorage.setItem("solcart_db_launches", JSON.stringify(defaultLaunches));
