          ...
          <div className="lg:col-span-2 space-y-4">
            <AnimatePresence initial={false}>
              {cartItems.map((item) => {
                ...
                const solItemEquivalent = parseFloat((itemPrice / solPrice).toFixed(4));
                return (
                  <motion.div 
                    key={item.itemKey}
                    ...
                    <div className="rounded-2xl border border-brand-border/40 bg-brand-card/20 p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          {item.selectedRegion && (
                            <p className="text-[10px] text-brand-text-muted mt-1">Region: <span className="text-white font-medium">{item.selectedRegion}</span>{item.selectedCurrency ? ` · ${item.selectedCurrency}` : ""}</p>
                          )}
