                    ...
                    >
                      ...
                      <div className="shrink-0">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            const products = o.items.map(it => it.productName).join(", ");
                            window.location.href = `mailto:support@solcart.shop?subject=Feedback%20for%20order%20${o.id}&body=Hi%20SOLCart%2C%0A%0AHere%20is%20my%20feedback%20on%20my%20recent%20purchase%20(${encodeURIComponent(products)}):%0A%0A`;
                          }}
                          className="px-3 py-2 rounded-lg bg-brand-purple/10 hover:bg-brand-purple/25 border border-brand-purple/30 text-[10px] font-bold text-brand-purple transition-all whitespace-nowrap"
                        >
                          Request Feedback
                        </button>
                      </div>
              ...
              <div className="glass-panel border border-brand-border/40 rounded-2xl p-5 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-brand-text-muted mt-0.5">The storefront uses a custom high-contrast style guide.</p>
                  </div>
                </div>

                <div className="p-4 bg-black border border-[#FF0000]/30 rounded-xl flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full bg-[#FF0000] animate-pulse"></span>
                    <span className="text-[10px] font-black text-white uppercase tracking-wider">
                      Ferrari Red & Black (Active)
                    </span>
                  </div>
                  ...
                  {selectedOrder.items.map((it, idx) => (
                    <div key={idx} className="flex justify-between items-center text-xs text-white">
                      <span>
                        ...
                        {it.selectedRegion && <span className="text-brand-text-muted text-[10px]"> · {it.selectedRegion}{it.selectedCurrency ? ` · ${it.selectedCurrency}` : ""}</span>}
                      </span>
