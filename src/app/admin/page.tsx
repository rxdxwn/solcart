...
export default function AdminDashboard() {
  ...
  // Gift Card Pool state
  const [giftCardPool, setGiftCardPool] = useState<any[]>([]);
  const [newCodeBrand, setNewCodeBrand] = useState("Amazon");
  const [newCodeRegion, setNewCodeRegion] = useState("United States");
  ...
  const getStatusBadge = (status: string | undefined | null) => {
    if (!status) return 'bg-brand-border/10 text-brand-text-muted border border-white/5';
    switch (status.toLowerCase()) {
      ...
      case 'good':
      case 'excellent':
        return 'bg-white/5 border border-white/5 text-[#92c1e3] text-[#92c1e3] border border-brand-green/20';
      case 'shipped':
      case 'processing':
      case 'approved':
      ...
      default:
        return 'bg-[#080808] text-brand-text-muted border border-white/5';
    }
  };
  ...
  const handleAutoAssignGiftCardCode = (orderId: string) => {
    if (!selectedOrder) return;
    const firstItem = selectedOrder.items[0];
    if (!firstItem) return;
    // Allocate a region/price-aware code for the first purchased item,
    // then immediately deliver it to the customer.
    const code = allocateGiftCardCode(
      firstItem.productId,
      (firstItem as any).selectedRegion || "United States",
      firstItem.retailPriceUSD
    );
    if (!code) {
      alert("No gift card codes available in the pre-saved inventory for this product, region, and price. Please enter a code manually to fulfill this order.");
      return;
    }
    setGiftCardCodeInput(code);
    setTimeout(() => handleDeliverGiftCardCode(orderId), 0);
  };
  ...
  const getAggregatedCustomers = () => {
    ...
    orders.forEach(o => {
      ...
      if (o.customerDetails.email && o.customerDetails.email !== cust.email && !cust.email.includes("@solcart-user.shop")) {
  ...
  if (!isStaff) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#020202] px-4 py-12 sm:px-6 lg:px-8 font-sans">
        <div className="w-full max-w-md space-y-8 bg-[#080808] border border-white/5 border border-white/5 p-8 rounded-2xl relative shadow-2xl">
          <div className="text-center">
            <div className="mx-auto h-12 w-12 rounded-xl bg-white/5 border border-white/5 text-[#e2c190] border border-white/5 flex items-center justify-center text-[#e2c190] mb-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="email"
                  required
                  placeholder="admin@solcart.shop"
                  value={adminEmail}
                  onChange={(e) => setAdminEmail(e.target.value)}
                  className="w-full h-10 px-3.5 rounded-lg border border-white/5 bg-[#0c0c0e] border border-white/5 text-xs text-white placeholder-white/20 focus:outline-none focus:border-white/5"
                />
              </div>
              <div>
  ...
  return (
    <div className="flex min-h-screen bg-[#020202] text-white font-sans overflow-hidden">
      ...
      {/* SIDEBAR NAVIGATION */}
      <aside className="hidden md:flex flex-col w-64 bg-[#080808]/90 border-r border-white/5 shrink-0 select-none">
        
        {/* Sidebar Header */}
        <div className="flex items-center justify-between px-6 h-16 border-b border-white/5">
          <div className="flex items-center gap-3">
            <Activity className="h-6 w-6 text-[#e2c190]" />
          ...
          <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-white/10 border border-white/5 text-[#e2c190] text-[#e2c190] border border-white/5">
            {APP_VERSION}
          </span>
        </div>
        ...
        <div className="p-4 border-t border-white/5 bg-[#020202]/30 border border-white/5 flex items-center justify-between gap-2 text-xs">
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1 text-[10px] font-semibold text-[#e2c190]">
              <span className="h-1.5 w-1.5 rounded-full bg-white text-black animate-pulse"></span>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        {/* TOP BAR */}
        <header className="h-16 border-b border-white/5 bg-[#080808]/30 flex items-center justify-between px-6 shrink-0 z-20">
          
          <div className="flex items-center gap-4 flex-1">
            <span className="md:hidden font-black text-xs text-[#e2c190] uppercase">SOLCart Ops</span>
            {/* Quick Search */}
            <div className="relative max-w-xs w-full hidden sm:block">
              <input
                ...
                className="w-full h-8 pl-8 pr-3 text-xs bg-[#0c0c0e] border border-white/5 border border-white/5 rounded-lg text-white placeholder-brand-text-muted focus:outline-none focus:border-white/10/50 focus:ring-1 focus:ring-brand-purple/10"
              />
          ...
          <div className="flex items-center gap-4 text-xs font-semibold">
            ...
            <div className="relative">
              <button 
                ...
                className="relative p-1.5 rounded-lg hover:bg-[#080808]/60 text-brand-text-muted hover:text-white transition-colors"
              >
                <Bell className="h-4.5 w-4.5" />
                <span className="absolute top-1 right-1 h-1.5 w-1.5 rounded-full bg-white text-black"></span>
              </button>
              
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-72 rounded-xl border border-white/5 bg-[#080808] p-3 shadow-xl backdrop-blur-lg z-30 space-y-2.5">
                  <div className="flex justify-between items-center border-b border-white/5 pb-1.5">
                    ...
                    {notifications.length > 0 && (
                      <button 
                        ...
                        className="text-[9px] font-bold text-[#e2c190] hover:underline"
                      >
                        Clear All
                      </button>
                    )}
        ...
        <main className="p-6 space-y-8 flex-1">
          ...
          {!hasPermission(activeTab, "view") ? (
            <div className="mx-auto max-w-md px-6 py-20 flex flex-col items-center justify-center text-center bg-[#080808]/40 border border-white/5 rounded-3xl backdrop-blur-lg">
              ...
              <div className="text-xs bg-[#020202] border border-white/5 px-4 py-2.5 rounded-xl font-mono text-left max-w-xs">
                <p className="font-bold text-[#e2c190] uppercase tracking-wider text-[10px]">Required Scope:</p>
            ...
            <div className="w-full space-y-8 animate-fade-in-slow">
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    <h1 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
                      <LayoutDashboard className="h-6 w-6 text-[#e2c190]" />
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {/* Financial Metrics */}
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                    ...
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                      ...
                      {dataLoading ? (
                        ...
                        <p className="text-lg sm:text-2xl font-black text-[#92c1e3] mt-1.5">${netProfitUSD.toFixed(2)}</p>
                      )}
                    </div>
                    ...
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                      ...
                      {dataLoading ? (
                        ...
                        <p className="text-lg sm:text-2xl font-black text-[#92c1e3] mt-1.5">{completedOrders}</p>
                      )}
                    </div>
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                    ...
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                      ...
                      {dataLoading ? (
                        ...
                        <p className="text-lg sm:text-2xl font-black text-[#92c1e3] mt-1.5">${totalFEEUSD.toFixed(2)}</p>
                      )}
                    </div>
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                    ...
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                      ...
                      {dataLoading ? (
                        ...
                        <p className="text-lg sm:text-2xl font-black text-[#92c1e3] mt-1.5">{paymentSuccessRate}</p>
                      )}
                    </div>
                    <div className="bg-[#080808] border border-white/5 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    {/* Revenue Trends */}
                    <div className="bg-[#080808] border border-white/5 rounded-2xl p-5 border border-white/5">
              ...
              {activeTab === "orders" && (
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <select
                          ...
                          className="h-8 px-2.5 bg-[#0c0c0e] border border-white/5 border border-white/5 rounded-lg text-xs font-bold text-white cursor-pointer focus:outline-none hover:border-white/5"
                        >
                  ...
                  {/* Orders Data Table */}
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-[#0c0c0e]/80 text-brand-text-muted border-b border-white/5">
                        <tr>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {filteredOrders.map(order => (
                          <tr key={order.id} className="hover:bg-[#080808]/25 transition-colors">
                            ...
                            <td className="p-3.5 font-semibold text-[#92c1e3]">
                            ...
                            <td className="p-3.5 text-right">
                              <button
                                ...
                                className="px-3 py-1.5 bg-white/10 border border-white/5 text-[#e2c190] text-[#e2c190] hover:bg-white/10 rounded-lg border border-white/5 font-bold flex items-center gap-1.5 ml-auto"
                              >
                  ...
                  {selectedOrder && (
                    <div className="fixed inset-0 bg-[#020202]/70 backdrop-blur-sm z-50 flex justify-end animate-fade-in font-sans">
                      <div className="w-full max-w-lg bg-[#080808] border-l border-white/5 h-full p-6 overflow-y-auto space-y-6 flex flex-col justify-between shadow-2xl">
                        ...
                        <div className="space-y-6">
                          <div className="flex justify-between items-center border-b border-white/5 pb-4">
                            ...
                            <button 
                              ...
                              className="p-1.5 rounded-lg hover:bg-[#0c0c0e]/60 text-brand-text-muted hover:text-white"
                            >
                              <X className="h-5 w-5" />
                            </button>
                          </div>
                              ...
                              {isEditingCustomer ? (
                                <div className="space-y-2 mt-1">
                                  <input
                                    ...
                                    className="w-full px-2.5 py-1 bg-[#020202] border border-white/5 rounded text-xs text-white"
                                  ...
                                  <div className="flex gap-1.5">
                                    ...
                                    <button
                                      ...
                                      className="px-2 py-0.5 rounded bg-[#020202] border border-white/5 text-[10px] font-bold text-white hover:bg-brand-border"
                                    >
                                      Cancel
                                    </button>
                                  </div>
                                ...
                                <div className="flex items-start gap-1.5 mt-1">
                                  ...
                                  <button
                                    ...
                                    className="text-[10px] font-bold text-[#e2c190] hover:underline"
                                  >
                                    Edit
                                  </button>
                                </div>
                          ...
                          <div className="space-y-2.5">
                            ...
                            {selectedOrder.items.map((item, idx) => (
                              <div key={idx} className="flex justify-between items-center p-3 rounded-xl border border-white/5 bg-[#020202]/30">
                                <div>
                          ...
                          {/* Transactions hash */}
                          <div className="p-4 rounded-xl border border-white/5 bg-[#020202]/30 border border-white/5 text-[10px] font-mono space-y-1.5">
                            <p className="font-bold text-[#e2c190] uppercase tracking-wider text-[9px] font-sans">Blockchain logs</p>
                            ...
                            <p className="truncate text-brand-text-muted">Swap Status: <span className="text-[#92c1e3] font-semibold">{"SOL -> USDC Swapped (100%)"}</span></p>
                          </div>

                          {/* Actions / Update panel */}
                          {hasPermission("orders", "edit") && (
                            <div className="space-y-3 p-4 rounded-xl border border-white/5 bg-white/5">
                              <p className="font-bold text-[#e2c190] uppercase tracking-wider text-[9px] font-sans">Administrative Actions</p>
                              ...
                              {selectedOrder.status !== "delivered" && selectedOrder.status !== "refunded" ? (
                                <div className="space-y-2">
                                  ...
                                  <div className="flex gap-2">
                                    <input
                                      ...
                                      className="flex-1 px-3 py-1.5 text-xs bg-[#020202] border border-white/5 rounded-lg text-white"
                                    />
                                    <button
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <p className="p-2.5 rounded bg-[#0c0c0e]/60 border border-white/5 font-mono text-xs text-[#92c1e3] font-bold select-all tracking-wider text-center">
                          ...
                          <div className="space-y-3 pt-2">
                            ...
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                              {logs.filter(l => l.details.includes(selectedOrder.id)).map(l => (
                                <div key={l.id} className="p-2 bg-[#0c0c0e] border border-white/5 rounded-lg text-[10px] text-brand-text-muted">
                            ...
                            <div className="flex gap-2">
                              <input
                                ...
                                className="flex-1 px-3 py-1.5 text-xs bg-[#020202] border border-white/5 rounded-lg text-white focus:outline-none focus:border-white/10"
                              />
                              <button
                                onClick={() => handleAddInternalNote(selectedOrder.id)}
                                className="px-3 bg-[#080808] hover:bg-brand-border border border-white/5 text-xs font-bold text-white rounded-lg"
                              >
                                Add
                              </button>
                            </div>
                        ...
                        <div className="pt-6 border-t border-white/5 flex justify-between gap-4">
                          <button
                            ...
                            className="flex-1 py-2 border border-white/5 bg-[#080808] text-xs font-bold hover:bg-brand-border rounded-lg text-white flex items-center justify-center gap-1.5"
                          >
              ...
              {activeTab === "analytics" && (
                <div className="space-y-8 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-[#080808] border border-white/5 p-5 rounded-2xl border border-white/5 text-center">
                      <span className="text-[10px] font-bold text-brand-text-muted uppercase">Cumulative gross profit</span>
                      <p className="text-2xl font-black text-[#92c1e3] mt-2">${netProfitUSD.toFixed(2)}</p>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {getAggregatedCustomers().length === 0 ? (
                          ...
                          getAggregatedCustomers().map((cust, idx) => (
                            <tr key={idx} className="hover:bg-[#080808]/25">
                              ...
                              <td className="p-3.5 font-bold text-[#92c1e3]">${cust.totalSpend.toFixed(2)}</td>
                              <td className="p-3.5">
                                <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                  cust.status === "Verified" 
                                    ? "bg-white/5 border border-white/5 text-[#92c1e3] text-[#92c1e3] border border-brand-green/20" 
                                    : "bg-white/5 border border-white/5 text-[#e2c190] text-[#e2c190] border border-white/5"
                                }`}>
                                  {cust.status}
                                </span>
                              </td>
                  ...
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-white/5 font-mono text-[11px]">
                        {transactions.map(tx => (
                          <tr key={tx.id} className="hover:bg-[#080808]/25">
                            ...
                            <td className="p-3.5 text-[#e2c190] font-bold">{tx.token}</td>
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {refunds.map(ref => (
                          <tr key={ref.id} className="hover:bg-[#080808]/25">
                            ...
                            <td className="p-3.5 font-bold text-[#92c1e3]">{ref.paidSOL.toFixed(4)} SOL</td>
                            <td className="p-3.5">
                              ...
                              {ref.status === "pending" && hasPermission("refunds", "edit") ? (
                                <div className="flex gap-2 justify-end">
                                  <button
                                    ...
                                    className="px-2.5 py-1.5 bg-white/5 border border-white/5 text-[#92c1e3] text-[#92c1e3] border border-brand-green/30 hover:bg-white text-black/30 rounded-lg font-bold"
                                  >
                                    Approve SOL Refund
                                  </button>
                                  <button
              ...
              {activeTab === "retailers" && (
                <div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15 max-w-4xl font-sans">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-[#0c0c0e]/80 text-brand-text-muted border-b border-white/5">
                        <tr>
                          <th className="p-3.5">Retailer</th>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {retailers.map(ret => (
                          <tr key={ret.id} className="hover:bg-[#080808]/25">
                            <td className="p-3.5 font-bold text-white uppercase">{ret.name}</td>
                            <td className="p-3.5 font-black text-[#e2c190]">
                              {editingRetailerId === ret.id ? (
                                <div className="flex items-center gap-2">
                                  <input
                                    ...
                                    className="w-16 px-2 py-1 text-xs bg-[#020202] border border-white/5 rounded text-white"
                                  />
                                  <span>%</span>
                                </div>
                              ) : (
                              ...
                              {hasPermission("retailers", "edit") && (
                                editingRetailerId === ret.id ? (
                                  <button
                                    ...
                                    className="px-2.5 py-1.5 bg-white/5 border border-white/5 text-[#92c1e3] text-[#92c1e3] border border-brand-green/30 rounded-lg font-bold"
                                  >
                                    Save
                                  </button>
                                ) : (
                                  <button
                                    ...
                                    className="px-2.5 py-1.5 bg-[#080808] hover:bg-brand-border border border-white/5 text-white rounded-lg font-bold"
                                  >
                                    Adjust
                                  </button>
                                )
...
{/* Add Product Form */}
                  {showProductForm && (
                    <form onSubmit={handleAddProduct} className="p-5 rounded-2xl border border-white/5 bg-[#080808]/25 max-w-2xl space-y-4 animate-fade-in text-xs">
                      <h3 className="text-xs font-bold text-[#e2c190] uppercase tracking-wider">
                      ...
                      <div className="flex gap-4">
                        ...
                        <button
                          ...
                          className="px-6 py-2 bg-[#020202] hover:bg-brand-border border border-white/5 rounded-lg text-xs font-bold text-white"
                        >
                          Cancel
                        </button>
                      </div>
              ...
              {activeTab === "inventory" && (
                <div className="space-y-4 font-sans">
                              ...
                              return (
                                <tr key={prod.id} className="hover:bg-[#080808]/25">
                                  <td className="p-3.5 flex items-center gap-3">
                                    <div className="relative h-8 w-8 rounded overflow-hidden shrink-0 bg-[#020202]">
                                      <img
                                    ...
                                    {editingStockProductId === prod.id ? (
                                      <div className="inline-flex items-center gap-2">
                                        <input
                                          ...
                                          className="w-16 px-2 py-1 bg-[#020202] border border-white/5 rounded text-center text-white font-mono text-xs focus:outline-none"
                                        />
                                        <button
                                          onClick={async () => {
                                          ...
                                          className="px-2.5 py-1 bg-white hover:bg-white/90 text-black rounded text-[10px] font-bold text-brand-dark transition-colors"
                                        >
                                          Save
                                        </button>
                                        <button
                                          onClick={() => setEditingStockProductId(null)}
                                          className="px-2.5 py-1 bg-[#080808] hover:bg-brand-border border border-white/5 rounded text-[10px] font-bold text-white transition-colors"
                                        >
                                          Cancel
                                        </button>
                                      </div>
                                      ...
                                      <button
                                        ...
                                        className="px-3 py-1 bg-white/5 border border-white/5 text-[#e2c190] hover:bg-white/10 border border-white/5 text-[#e2c190] border border-white/10/25 rounded text-[10px] font-bold text-[#e2c190] transition-all"
                                      >
                                        Edit Stock
                                      </button>
                                    )}
                  ...
                  {/* Digital Gift Card Codes Pool */}
                  <div className="pt-6 border-t border-white/5 space-y-6">
                    <div>
                      <h2 className="text-sm font-bold text-white uppercase tracking-wider">Digital Gift Card Codes Pool</h2>
                      <p className="text-[11px] text-brand-text-muted mt-1">Pre-save gift card codes for automated instant delivery upon checkout. If the pool is empty for a particular region/price, checkouts go to pending status for manual fulfillment.</p>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      {/* Left: Add codes form */}
                      <div className="p-4 rounded-xl border border-white/5 bg-[#080808]/15 space-y-4 text-xs">
                        <p className="font-bold text-[#e2c190] uppercase tracking-wider text-[10px]">Add Codes in Bulk</p>
                        
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <label className="text-brand-text-muted font-bold block">Brand Name</label>
                            <select
                              value={newCodeBrand}
                              onChange={(e) => setNewCodeBrand(e.target.value)}
                              className="w-full px-3 py-1.5 bg-[#020202] border border-white/5 rounded-lg text-white"
                            >
                          ...
                          <div className="space-y-1">
                            <label className="text-brand-text-muted font-bold block">Region</label>
                            <input
                              type="text"
                              required
                              placeholder="e.g. United States"
                              value={newCodeRegion}
                              onChange={(e) => setNewCodeRegion(e.target.value)}
                              className="w-full px-3 py-1.5 bg-[#020202] border border-white/5 rounded-lg text-white"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="text-brand-text-muted font-bold block">Price Point ($ USD)</label>
                            <input
                              type="number"
                              required
                              placeholder="e.g. 50"
                          ...
                          <div className="space-y-1">
                            <label className="text-brand-text-muted font-bold block">Paste Gift Card Codes (one per line)</label>
                            <textarea
                              rows={5}
                              required
                              placeholder="AMZN-US-50-ABCD1234&#10;AMZN-US-50-WXYZ5678"
                              value={newCodesTextArea}
                              onChange={(e) => setNewCodesTextArea(e.target.value)}
                              className="w-full px-3 py-1.5 bg-[#020202] border border-white/5 rounded-lg text-white font-mono resize-none"
                            />
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            if (!newCodesTextArea.trim()) return;
                            const lines = newCodesTextArea.split("\n").map(l => l.trim()).filter(Boolean);
                            if (lines.length === 0) return;
                            
                            const pool = SupabaseService.getGiftCardPool();
                            let lastId = pool.length > 0 ? Math.max(...pool.map((c: any) => parseInt(c.id.replace("gc-", "")) || 0)) : 0;
                            
                            const newEntries = lines.map(code => ({
                              id: `gc-${++lastId}`,
                              brand: newCodeBrand,
                              region: newCodeRegion,
                              price: parseInt(newCodePrice) || 0,
                              code,
                              isUsed: false,
                              createdAt: new Date().toISOString()
                            }));
                            
                            const updatedPool = [...pool, ...newEntries];
                            SupabaseService.saveGiftCardPool(updatedPool);
                            setGiftCardPool(updatedPool);
                            setNewCodesTextArea("");
                      ...
                      <div className="lg:col-span-2 space-y-3">
                        <div className="flex justify-between items-center">
                          <p className="font-bold text-white uppercase tracking-wider text-[10px]">Active Inventory Pool ({giftCardPool.length} codes)</p>
                          <input
                            type="text"
                            placeholder="Filter by brand/region/code..."
                            value={poolSearchQuery}
                            onChange={(e) => setPoolSearchQuery(e.target.value)}
                            className="px-2.5 py-1 text-[11px] bg-[#020202] border border-white/5 rounded-lg text-white w-48 focus:outline-none"
                          />
                        </div>

                        <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15 max-h-96 overflow-y-auto">
                          <table className="w-full text-left text-xs">
                            <thead className="bg-[#0c0c0e]/80 text-brand-text-muted border-b border-white/5 sticky top-0 z-10">
                              <tr>
                            ...
                            <tbody className="divide-y divide-white/5">
                              {giftCardPool.length === 0 ? (
                                <tr>
                                  <td colSpan={6} className="p-8 text-center text-brand-text-muted">
                                    No gift card codes in the pool. Use the form to seed your inventory.
                                  </td>
                                </tr>
                              ) : (
                                giftCardPool
                                  ...
                                  .map(item => (
                                    <tr key={item.id} className="hover:bg-[#080808]/25 text-[11px]">
                                      <td className="p-2.5 font-bold text-white">{item.brand}</td>
                                      <td className="p-2.5 text-brand-text-muted">{item.region}</td>
                                      <td className="p-2.5 text-white font-bold font-mono">${item.price}</td>
                                      <td className="p-2.5 font-mono select-all text-white font-semibold">{item.code}</td>
                                      <td className="p-2.5 font-bold">
                                        {item.isUsed ? (
                                          <span className="px-1.5 py-0.5 rounded text-[9px] uppercase font-black tracking-wider text-brand-text-muted bg-white/5 border border-white/5">
                                            Used
                                          </span>
                                        ) : (
                                          <span className="px-1.5 py-0.5 rounded text-[9px] uppercase font-black tracking-wider text-brand-green bg-brand-green/10 border border-brand-green/20">
                                            Available
                                          </span>
                                        )}
                                      </td>
                                        ...
                                        <button
                                          onClick={() => {
                                            if (confirm("Are you sure you want to delete this code?")) {
                                              const updated = giftCardPool.filter(c => c.id !== item.id);
                                              SupabaseService.saveGiftCardPool(updated);
                                              setGiftCardPool(updated);
                                              refreshAllData();
                                            }
                                          }}
              ...
              {activeTab === "staff" && (
                <div className="space-y-4">
                  ...
                  {/* Add Staff form */}
                  {showStaffForm && (
                    <form onSubmit={handleAddStaff} className="p-5 rounded-2xl border border-white/5 bg-[#080808]/25 max-w-md space-y-4 animate-fade-in text-xs">
                      <h3 className="text-xs font-bold text-[#e2c190] uppercase tracking-wider">Staff Account Invite</h3>
                      <div className="space-y-3">
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            type="email"
                            required
                            placeholder="e.g. john@solcart.shop"
                            value={newStaffEmail}
                      ...
                      <div className="flex gap-4">
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-[#020202] border border-white/5 text-xs font-bold text-white rounded-lg"
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {staff.map(member => (
                          <tr key={member.id} className="hover:bg-[#080808]/25">
                            <td className="p-3.5 font-bold text-white">{member.name}</td>
                            <td className="p-3.5 text-brand-text-muted">{member.email}</td>
                            <td className="p-3.5 font-bold text-[#e2c190] text-[10px] tracking-wide uppercase">{member.role}</td>
                            <td className="p-3.5">
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {tickets.map(tkt => (
                          <tr key={tkt.id} className="hover:bg-[#080808]/25">
                            ...
                            <td className="p-3.5 font-bold text-[#e2c190] text-[10px] uppercase">{tkt.assignedTo || "Unassigned"}</td>
                            <td className="p-3.5">
                            ...
                            <td className="p-3.5 text-right">
                              <button
                                ...
                                className="px-3 py-1.5 bg-white/10 border border-white/5 text-[#e2c190] text-[#e2c190] border border-white/5 hover:bg-white text-black/30 rounded-lg font-bold ml-auto"
                              >
                                View Ticket
                              </button>
                            </td>
                  ...
                  {selectedTicket && (
                    <div className="fixed inset-0 bg-[#020202]/70 backdrop-blur-sm z-50 flex justify-center items-center p-4">
                      <div className="w-full max-w-lg bg-[#080808] border border-white/5 rounded-3xl p-6 space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl animate-fade-in-slow text-xs">
                        ...
                        <div className="flex justify-between items-start border-b border-white/5 pb-3">
                          ...
                          <button 
                            ...
                            className="p-1 rounded-lg hover:bg-[#020202] text-brand-text-muted hover:text-white"
                          >
                            <X className="h-5 w-5" />
                          </button>
                        </div>

                        <div className="p-4 rounded-xl border border-white/5 bg-[#020202]/30">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 max-h-40 overflow-y-auto">
                            {selectedTicket.comments?.map((c: any, index: number) => (
                              <div key={index} className="p-2.5 bg-[#020202]/30 border border-white/5 border border-white/5 rounded-lg">
                                <div className="flex justify-between font-bold text-[10px] text-[#e2c190] uppercase">
                                  <span>{c.author}</span>
                          ...
                          <form onSubmit={handleTicketComment} className="flex gap-2">
                            <input
                              ...
                              className="flex-1 px-3 py-2 bg-[#020202] border border-white/5 rounded-lg text-xs text-white"
                            />
                            <button
                              type="submit"
                        ...
                        <div className="flex justify-between items-center pt-4 border-t border-white/5">
                          {selectedTicket.status !== "resolved" ? (
                            ...
                            <span className="text-[#92c1e3] font-bold text-xs flex items-center gap-1.5">
                          ...
                          <button
                            onClick={() => setSelectedTicket(null)}
                            className="px-4 py-2 bg-[#020202] border border-white/5 text-xs font-bold hover:bg-brand-border rounded-lg text-white"
                          >
                            Close
                          </button>
                        </div>
                  ...
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-[#080808]/15">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-white/5 font-mono text-[10px]">
                        {logs.map(log => (
                          <tr key={log.id} className="hover:bg-[#080808]/40 border border-white/5">
                            <td className="p-3.5 text-brand-text-muted">
                              {new Date(log.timestamp).toLocaleString()}
                            </td>
                            <td className="p-3.5 font-bold text-[#e2c190] uppercase">
                              {log.action}
                            </td>
              ...
              {activeTab === "settings" && (
                <div className="space-y-6 max-w-2xl">
                  ...
                  <div className="p-6 rounded-2xl border border-white/5 bg-[#080808]/15 space-y-6 text-xs font-sans">

                    {/* USDC Associated Token Account Initialization Status */}
                    <div className="p-4 rounded-xl border border-white/5 bg-[#020202]/30 border border-white/5 space-y-3">
                      <div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                        <span className="text-[10px] text-brand-text-muted font-mono bg-[#020202] px-2 py-1 rounded border border-white/5">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-white text-black hover:bg-white text-black/80 text-white text-[10px] font-bold transition-all text-center whitespace-nowrap"
                        >
                          Initialize USDC ATA
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg text-xs font-bold text-white transition-all ${
                          settings.maintenanceMode 
                            ...
                            : 'bg-[#080808] hover:bg-brand-border border border-white/5'
                        }`}
                      >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="w-16 px-2 py-1 bg-[#020202] border border-white/5 rounded text-center text-white"
                        />
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          className="w-20 px-2 py-1 bg-[#020202] border border-white/5 rounded text-center text-white font-mono"
                        />
                      </div>
                    </div>
