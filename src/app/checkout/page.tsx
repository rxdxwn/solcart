...
import { allocateGiftCardCode, getRemainingCodeCount } from "../../data/inventory";
...
export default function CheckoutPage() {
  ...
  const [finalOrderStatus, setFinalOrderStatus] = useState<'delivered' | 'pending'>('delivered');
  ...
  const executeCheckoutPayment = async () => {
    ...
    try {
      ...
      const orderItems = cartItems.map(item => ({
        ...
        retailPriceUSD: item.selectedPrice ?? item.product.retailPrice,
        marketplacePriceUSD: item.selectedPrice ?? item.product.retailPrice,
        image: item.product.image,
        selectedRegion: item.selectedRegion || "United States",
        selectedCurrency: item.selectedCurrency || "USD"
      }));

      // Check if all items in the cart are in stock in the pre-saved inventory
      let allInStock = true;
      cartItems.forEach(item => {
        const remainingCount = getRemainingCodeCount(
          item.product.id,
          item.selectedRegion || "United States",
          item.selectedPrice ?? item.product.retailPrice
        );
        if (remainingCount <= 0) {
          allInStock = false;
        }
      ...
      let mockCode = "";
      let orderStatus: 'delivered' | 'pending' = 'delivered';
      const allocatedCodes: string[] = [];

      if (allInStock) {
        // Allocate real gift card codes per cart item
        cartItems.forEach(item => {
        ...
        mockCode = allocatedCodes[0] || "";
        orderStatus = 'delivered';
        setClaimedCode(mockCode);
        setFinalOrderStatus('delivered');
      } else {
        // Mark order as pending manual fulfillment
        mockCode = "";
        orderStatus = 'pending';
        setClaimedCode("");
        setFinalOrderStatus('pending');
      }

      const primaryRetailerId = cartItems[0]?.product.retailerId || "amazon";
      // Region for the digital delivery address is derived from the cart selection
      const digitalRegion = cartItems[0]?.selectedRegion || "United States";
      ...
      const createdOrder = SupabaseService.createOrder({
        ...
        txHash,
        swapTxHash,
        giftCardCode: mockCode || undefined,
        status: orderStatus
      });
      ...
      // Dispatch gift card code delivery emails ONLY if order is instantly delivered
      if (orderStatus === 'delivered' && allocatedCodes.length > 0) {
        allocatedCodes.forEach((code, idx) => {
          fetch("/api/email/delivery", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              toEmail: custEmail,
          ...
          }).catch(err => console.warn(`Failed to send gift card delivery email for item ${idx}`, err));
        });
      }
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 w-full relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Gift Card Region summary (locked per cart item at selection time) */}
            ...
            <div className="space-y-2.5">
              {cartItems.map(item => (
                <div key={item.itemKey} className="flex items-center justify-between text-xs">
                  <span className="text-brand-text-muted truncate pr-3">
                    {item.product.name}
                  </span>
                  <span className="text-white font-bold shrink-0">
                    {item.selectedRegion || "United States"}
                    {item.selectedCurrency ? ` · ${item.selectedCurrency}` : ""}
                  </span>
                </div>
              ))}
              <p className="text-[10px] text-brand-text-muted mt-2 leading-relaxed">
                * Region was selected on the product page and is locked per item. Gift card code activation regions match the region selected for each item.
              </p>
            </div>
          </div>
        ...
        <div className="glass-panel rounded-2xl p-6 border border-brand-border/40 space-y-6">
          ...
          <div className="max-h-44 overflow-y-auto space-y-3.5 pr-2">
            {cartItems.map(item => {
              const itemPrice = item.selectedPrice ?? item.product.retailPrice;
              return (
                <div key={item.itemKey} className="flex justify-between items-center gap-3 text-xs">
                  <div className="flex items-center gap-2">
                    <div className="relative h-7 w-11 shrink-0">
                      <GiftCardArtwork brand={item.product.brand} value={itemPrice} imageUrl={item.product.image} className="shadow-md" isThumbnail={true} />
                    </div>
                    <div>
                      <p className="font-bold text-white truncate max-w-[130px]">{item.product.name}</p>
                      <p className="text-[10px] text-brand-text-muted mt-0.5">Qty: {item.quantity} x ${itemPrice}{item.selectedRegion ? ` · ${item.selectedRegion}` : ""}</p>
                    </div>
                  </div>
                  <span className="font-bold text-white">${(itemPrice * item.quantity).toFixed(2)}</span>
                </div>
              );
            })}
          </div>
            ...
            {paymentStep === 'success' ? (
              <div className="space-y-5 pt-2">
                ...
                {finalOrderStatus === 'delivered' ? (
                  ...
                  /* Manual Fulfillment Pending Notice */
                  <div className="relative rounded-2xl border border-amber-500/30 bg-gradient-to-tr from-brand-card to-amber-950/20 p-6 text-center overflow-hidden shadow-2xl">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/5 rounded-full blur-2xl pointer-events-none"></div>
                    
                    <div className="h-10 w-10 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 mx-auto mb-3">
                      <Clock className="h-5 w-5 animate-pulse" />
                    </div>
                    
                    <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest">Manual Fulfillment Pending</p>
                    
                    <p className="my-3 text-xs text-white leading-relaxed font-sans px-2">
                      Your payment has been successfully settled! Since this specific gift card is currently out of instant stock, our admin team is preparing your code manually.
                    </p>
                    
                    <p className="text-[10px] text-brand-text-muted leading-relaxed border-t border-brand-border/40 pt-3">
                      Your code will be delivered directly to your email <span className="text-white font-bold">{custEmail}</span> and uploaded to your Customer Dashboard as soon as it is processed.
                    </p>
                  </div>
                )}
