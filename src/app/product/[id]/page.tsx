...
export default function ProductDetailPage({ params }: PageProps) {
  ...
  // Region & price point selection (captured BEFORE adding to cart)
  ...
  useEffect(() => {
    const handleSync = () => {
      ...
      if (prod) {
        setProduct(prod);
        // Initialize region/price selections (kept stable once the user picks)
        setSelectedRegion(prev => prev || prod.regions?.[0] || "United States");
        setSelectedPrice(prev => prev ?? prod.retailPrice);
      }
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        ...
        <div className="flex flex-col gap-4">
          <div className="p-8 aspect-[4/3] rounded-3xl border border-brand-border/40 bg-gradient-to-br from-brand-card/50 to-indigo-950/20 flex items-center justify-center relative overflow-hidden shadow-2xl group">
            ...
            <GiftCardArtwork brand={product.brand} value={effectivePrice} imageUrl={product.image} className="shadow-2xl max-w-[90%] transform group-hover:scale-[1.01] transition-transform duration-500" />
