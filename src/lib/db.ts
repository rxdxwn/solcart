...
// =========================================================================
// UNIFIED DATABASE ADAPTER CLASS
// =========================================================================
  ...
  static async getSettings(): Promise<any> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getSettings failed, falling back to local DB", e);
      }
    }
  ...
  static async updateSettings(payload: any): Promise<any> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase updateSettings failed, falling back to local DB", e);
      }
    }
  ...
  static async getProducts(): Promise<Product[]> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getProducts failed, falling back to local DB", e);
      }
    }
  ...
  static async addProduct(payload: Omit<Product, "rating" | "reviewsCount" | "reviews" | "marketplacePrice"> & { marketplacePrice?: number }): Promise<Product> {
    ...
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase addProduct failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async deleteProduct(productId: string): Promise<boolean> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase deleteProduct failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async updateProductStock(productId: string, stockCount: number): Promise<boolean> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase updateProductStock failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async addProductReview(productId: string, author: string, rating: number, comment: string): Promise<any> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase addProductReview failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async getOrders(): Promise<Order[]> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getOrders failed, falling back to local DB", e);
      }
    }
  ...
  static async createOrder(order: Order): Promise<Order> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase createOrder failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async updateOrderStatus(orderId: string, status: string, details?: any): Promise<boolean> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase updateOrderStatus failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async deliverGiftCardCode(orderId: string, giftCardCode: string): Promise<boolean> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase deliverGiftCardCode failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
    const idx = store.orders.findIndex((o: any) => o.id === orderId);
  ...
  static async updateOrderCustomerName(orderId: string, customerName: string): Promise<boolean> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase updateOrderCustomerName failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
    const idx = store.orders.findIndex((o: any) => o.id === orderId);
  ...
  static async getTransactions(): Promise<Transaction[]> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getTransactions failed, falling back to local DB", e);
      }
    }
  ...
  static async createTransaction(tx: Transaction): Promise<Transaction> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase createTransaction failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async getUsers(): Promise<any[]> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getUsers failed, falling back to local DB", e);
      }
    }
  ...
  static async createUser(user: any): Promise<any> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase createUser failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async updateUser(email: string, updates: any): Promise<boolean> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase updateUser failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async getTickets(): Promise<any[]> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getTickets failed, falling back to local DB", e);
      }
    }
  ...
  static async createTicket(payload: any): Promise<any> {
    ...
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase createTicket failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async addTicketComment(ticketId: string, comment: string): Promise<any> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase addTicketComment failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
  ...
  static async getActivityLogs(): Promise<ActivityLog[]> {
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase getActivityLogs failed, falling back to local DB", e);
      }
    }
  ...
  static async logActivity(category: string, message: string, type: ActivityLog['type'] = "info", userId?: string): Promise<ActivityLog | null> {
    ...
    if (isSupabaseConfigured() && supabaseAdmin) {
      try {
        ...
        console.error("Supabase logActivity failed, falling back to local DB", e);
      }
    }
    const store = readLocalDb();
