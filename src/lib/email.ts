...
// =========================================================================
// OFFICIAL SOLCART EMAIL CONFIGURATION
// -------------------------------------------------------------------------
// The official SOLCart domain is solcart.shop (NOT .io). The from-address and
// support mailbox below are the canonical SOLCart addresses.
//
// NOTE: For production, set the SMTP credentials via environment variables
// (EMAIL_SMTP_USER / EMAIL_SMTP_PASS). The hardcoded values below are the
// current working credentials and should be replaced with the official
// SOLCart mailbox once provisioned.
// =========================================================================
export const SOLCART_SUPPORT_EMAIL = process.env.SOLCART_SUPPORT_EMAIL || "support@solcart.shop";
export const SOLCART_FROM_EMAIL = process.env.SOLCART_FROM_EMAIL || "no-reply@solcart.shop";
...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: process.env.EMAIL_SMTP_USER || "andriostest@gmail.com",
    pass: process.env.EMAIL_SMTP_PASS || "mobc cdmp yhhm deqr"
  }
});
 ...
 */
export async function sendEmail({ to, subject, html, bcc }: { to: string; subject: string; html: string; bcc?: string }): Promise<boolean> {
  try {
    const info = await transporter.sendMail({
      from: `"SOLCart" <${SOLCART_FROM_EMAIL}>`,
      to,
      bcc,
      subject,
      html
...
export async function sendOrderReceiptEmail(toEmail: string, order: any): Promise<boolean> {
  ...
  `;
  return sendEmail({ to: toEmail, subject, html, bcc: "solcart164@gmail.com" });
}
