...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const removeFromCart = (itemKey: string) => {
    const items = cartItems.filter(item => item.itemKey !== itemKey);
    saveCart(items);
  };
  ...
  // subtotal is based on the selected price point (falls back to retailPrice)
  const subtotalUSD = cartItems.reduce(
    (acc, item) => acc + (item.selectedPrice ?? item.product.retailPrice) * item.quantity,
    0
  );
