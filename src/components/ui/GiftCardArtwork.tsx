...
export function GiftCardArtwork({ brand, value, imageUrl, className = "", aspectRatio = "card", isThumbnail = false }: GiftCardArtworkProps) {
  ...
  const config: Record<string, {
    ...
    noon: {
      bg: "bg-gradient-to-br from-[#FFEB00] via-[#FFE500] to-[#E6CD00]",
      logoUrl: "https://upload.wikimedia.org/wikipedia/commons/c/c5/Noon.com_logo.svg",
      logoClass: "brightness-0 filter",
      label: "NOON DIGITAL GIFT CARD",
      accentColor: "#FFEB00"
    }
  };
    ...
    <div className={`relative ${fillMode ? "w-full h-full" : `w-full max-w-full ${aspectClass}`} rounded-xl ${cardConfig.bg} border border-white/10 overflow-hidden shadow-2xl p-3 sm:p-4 flex flex-col justify-between select-none group ${className}`}>
      ...
      <div className="flex justify-between items-start z-10">
        <div className="flex flex-col items-start">
          ...
          <span className="text-[8.5px] sm:text-[10px] font-bold text-white tracking-tight mt-0.5 sm:mt-1 leading-none">
            {cardConfig.label}
          </span>
        </div>
      ...
      {/* Center brand logo */}
      <div className="flex items-center justify-center py-1.5 sm:py-2 z-10 flex-1 min-h-0">
        {cardConfig.logoUrl ? (
          <img 
            src={cardConfig.logoUrl} 
            alt={`${brand} logo`}
            ...
            className={`h-6 sm:h-8 md:h-9 w-auto max-w-[70%] object-contain select-none pointer-events-none ${cardConfig.logoClass || ""}`}
          />
        ) : null}
        <span 
      ...
      <div className="flex justify-between items-center z-10 pt-1.5 border-t border-white/5">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[6.5px] sm:text-[7px] font-bold text-white/40 tracking-wider uppercase font-mono">
            INSTANT DELIVERY
          </span>
        </div>
        <div className="flex items-center text-[6.5px] sm:text-[7px] text-white/40 font-mono">
